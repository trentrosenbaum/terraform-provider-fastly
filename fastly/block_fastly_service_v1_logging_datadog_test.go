...
func TestAccFastlyServiceV1_logging_datadog_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccServiceV1DatadogWasmConfig(nameWasm, domain),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckServiceV1Exists("fastly_service_wasm_v1.foo", &service),
					testAccCheckFastlyServiceV1DatadogAttributes(&service, []*gofastly.Datadog{&log1Wasm}, ServiceTypeWasm),
					resource.TestCheckResourceAttr(
						"fastly_service_wasm_v1.foo", "name", nameWasm),
					resource.TestCheckResourceAttr(
						"fastly_service_wasm_v1.foo", "logging_datadog.#", "1"),
				),
			},
